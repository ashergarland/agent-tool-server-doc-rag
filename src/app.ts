...
import { LocalDocsProvider } from './provider/local-docs.js';
...
export interface CreateApplicationOptions {
  readonly config?: AppConfig;
  readonly logger?: Logger;
  readonly provider?: DocumentationProvider;
}

export const createApplication = (options: CreateApplicationOptions = {}): Application => {
  const config = options.config ?? loadConfig();
  const logger = options.logger ?? createLogger(config);
  const services = createServices(config, options.provider ?? new LocalDocsProvider(config.docs));
