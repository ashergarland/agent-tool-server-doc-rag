...
const server = (overrides: Record<string, unknown> = {}) => {
  ...
  const app = createHttpServer({
    ...
    services: createServices(config, new TestDocumentationProvider()),
...
describe('HTTP API', () => {
  ...
  it('invokes tools and maps validation failures', async () => {
    ...
    const success = await app.inject({
      method: 'POST',
      url: '/tools/search_local_docs',
      headers: { 'x-api-key': apiKey },
      payload: { query: 'Azure client' },
    });
    expect(success.statusCode).toBe(200);
    expect(success.json().result.results[0].source).toBe('azure/container-apps.md');
  ...
  it('publishes the generated OpenAPI document', async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.json().paths['/tools/search_local_docs']).toBeDefined();
  });
});
